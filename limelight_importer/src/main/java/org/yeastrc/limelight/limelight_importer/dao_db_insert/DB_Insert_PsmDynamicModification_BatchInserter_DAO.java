...
/**
 * Insert Batches into table psm_dynamic_modification_tbl 
 * 
 * DB_Insert_PsmDynamicModification_InsertOnly_DAO  
 *
 */
public class DB_Insert_PsmDynamicModification_BatchInserter_DAO implements DB_BatchInsert_ValidateCall_InsertLastBatch_ToDB_RegistryEntry_IF {

	private static final Logger log = LoggerFactory.getLogger( DB_Insert_PsmDynamicModification_BatchInserter_DAO.class );
	

	private static final int INSERT_BATCH_SIZE = 4000;

	
	private DB_Insert_PsmDynamicModification_BatchInserter_DAO() { 
	 ...
	 */
	public static DB_Insert_PsmDynamicModification_BatchInserter_DAO getSingletonInstance() { 
		return singletonInstance;
	}

	private static DB_Insert_PsmDynamicModification_BatchInserter_DAO singletonInstance = new DB_Insert_PsmDynamicModification_BatchInserter_DAO();
	
	private List<PsmDynamicModificationDTO> psmDynamicModificationDTOList = new ArrayList<>( INSERT_BATCH_SIZE );
	...
	public void insert_LAST_Batch_ToDB() throws Exception {
		...
		if ( ! psmDynamicModificationDTOList.isEmpty() ) {
	 ...
	 * @param psmDynamicModificationDTOList
	 * @throws Exception 
	 */
	public void insert_Batching_Object(

			PsmDynamicModificationDTO psmDynamicModificationDTO
...

		{
			...
			psmDynamicModificationDTOList.add(psmDynamicModificationDTO);
		}

		if ( psmDynamicModificationDTOList.size() >= INSERT_BATCH_SIZE ) {
	...
	private void _saveBatch() throws Exception {
		...
		{  //  Save  psmOpenModificationDTO objects
			...
			DB_Insert_PsmDynamicModification_InsertOnly_DAO.getInstance().insert_NOT_Update_ID_Property_InDTOParams(psmDynamicModificationDTOList);
		}

		//  Clear Batch List since All Saved

		psmDynamicModificationDTOList.clear();
	}
