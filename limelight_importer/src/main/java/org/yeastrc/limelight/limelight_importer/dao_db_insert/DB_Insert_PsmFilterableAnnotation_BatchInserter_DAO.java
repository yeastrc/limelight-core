...
/**
 * Insert Batches into table psm_filterable_annotation_tbl 
 * 
 * DB_Insert_PsmFilterableAnnotationDAO  
 *
 */
public class DB_Insert_PsmFilterableAnnotation_BatchInserter_DAO implements DB_BatchInsert_ValidateCall_InsertLastBatch_ToDB_RegistryEntry_IF {

	private static final Logger log = LoggerFactory.getLogger( DB_Insert_PsmFilterableAnnotation_BatchInserter_DAO.class );
	

	private static final int INSERT_BATCH_SIZE = 5000;

	
	private DB_Insert_PsmFilterableAnnotation_BatchInserter_DAO() { 
	 ...
	 */
	public static DB_Insert_PsmFilterableAnnotation_BatchInserter_DAO getSingletonInstance() { 
		return singletonInstance;
	}

	private static DB_Insert_PsmFilterableAnnotation_BatchInserter_DAO singletonInstance = new DB_Insert_PsmFilterableAnnotation_BatchInserter_DAO();
	
	private List<PsmFilterableAnnotationDTO> psmFilterableAnnotationDTOList = new ArrayList<>( INSERT_BATCH_SIZE );
	...
	public void insert_LAST_Batch_ToDB() throws Exception {
		...
		if ( ! psmFilterableAnnotationDTOList.isEmpty() ) {
	 ...
	 * @param psmFilterableAnnotationDTOList
	 * @throws Exception 
	 */
	public void insert_Batching_Object(

			PsmFilterableAnnotationDTO psmFilterableAnnotationDTO
...

		{
			...
			psmFilterableAnnotationDTOList.add(psmFilterableAnnotationDTO);
		}

		if ( psmFilterableAnnotationDTOList.size() >= INSERT_BATCH_SIZE ) {
	...
	private void _saveBatch() throws Exception {
		...
		{  //  Save  psmOpenModificationDTO objects
			...
			DB_Insert_PsmFilterableAnnotationDAO.getInstance().insert_NOT_Update_ID_Property_InDTOParams(psmFilterableAnnotationDTOList);
		}

		//  Clear Batch List since All Saved

		psmFilterableAnnotationDTOList.clear();
	}
