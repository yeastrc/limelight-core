 ...
 *
 */
public class ScanFileToSpectralStorageService_GetAPIKey__NO_SearchData {
	...
	private static final Logger log = LoggerFactory.getLogger( ScanFileToSpectralStorageService_GetAPIKey__NO_SearchData.class );
	
	private ScanFileToSpectralStorageService_GetAPIKey__NO_SearchData() { }
	public static ScanFileToSpectralStorageService_GetAPIKey__NO_SearchData getInstance() { return new ScanFileToSpectralStorageService_GetAPIKey__NO_SearchData(); }
...
//			SearchScanFileEntry searchScanFileEntry //  Overall data
		...
		while( true ) {  // use 'break;' inside loop to exit
			...
			if ( get_UploadedScanFileInfo_Response.getStatus() == WebserviceSpectralStorageAcceptImport_ProcessStatusEnum.FAIL ) {
				...
				{
					String scanFilename_WithLabel = ":  Uploaded Scan Filename: " + scanFileFileContainer.getScanFilename();
					
					String scanFileUploaded_OnDisk_WithLabel = ": Uploaded scan file on disk: "
								+ scanFileFileContainer.getScanFile().getAbsolutePath();
				...
				if ( scanFileParse_Data_ErrorMsg != null ) { // ABOVE: scanFileParse_Data_ErrorMsg = get_UploadedScanFileInfo_Response.getDataErrorFailMessage();
					...
					String scanFilename_WithLabel = ": Scan Filename: " + scanFileFileContainer.getScanFilename();
