/**
 * proteinViewPage_DisplayData_ProteinList__CreateProteinDisplayData_Compute_NSAF_Per_ExperimentConditionId.ts
 *
 ...
 *
 */
export const proteinViewPage_DisplayData_ProteinList__CreateProteinDisplayData_Compute_NSAF_Per_ExperimentConditionId = function (
    {
        proteinDisplayData,
    ...
    const psmCount_Map_Key_ProteinSequenceVersionId_Map_Key_ConditionId: Map<number, Map<number, number>> = new Map();
    ...
    if ( proteinDisplayData.proteinGroupsList ) {
        for (const proteinGroupItem of proteinDisplayData.proteinGroupsList) {
            ...
            _processProteinList_GetData_For_Compute_NSAF({
                ...
                psmCount_Map_Key_ProteinSequenceVersionId_Map_Key_ConditionId
            });
        }
    } else {
    ...
    for (const psmCount_Map_Key_ProteinSequenceVersionId_Map_Key_ConditionId_Entry of psmCount_Map_Key_ProteinSequenceVersionId_Map_Key_ConditionId.entries()) {

        const conditionId = psmCount_Map_Key_ProteinSequenceVersionId_Map_Key_ConditionId_Entry[0];
        const psmCount_Map_Key_ProteinSequenceVersionId_Map = psmCount_Map_Key_ProteinSequenceVersionId_Map_Key_ConditionId_Entry[1];
...
const _processProteinList_GetData_For_Compute_NSAF = function(
    {
        ...
        psmCount_Map_Key_ProteinSequenceVersionId_Map_Key_ConditionId: Map<number, Map<number, number>>
    ...
    for (const proteinItem of proteinList) {
        ...
        for ( const condition of conditions_with_their_project_search_ids_for_First_condition_group ) {
            ...
            let psmCount_Map_Key_ProteinSequenceVersionId_Map = psmCount_Map_Key_ProteinSequenceVersionId_Map_Key_ConditionId.get(condition.condition.id);
            if ( ! psmCount_Map_Key_ProteinSequenceVersionId_Map ) {
                psmCount_Map_Key_ProteinSequenceVersionId_Map = new Map();
                psmCount_Map_Key_ProteinSequenceVersionId_Map_Key_ConditionId.set(condition.condition.id, psmCount_Map_Key_ProteinSequenceVersionId_Map);
            }
            ...
            psmCount_Map_Key_ProteinSequenceVersionId_Map.set(proteinSequenceVersionId, experiment_SubData_ForCondition.numPsms);
        }
    }
}
...
const _processProteinList_Update_With_NSAF_Values = function(
    ...
    for (const proteinItem of proteinList) {
        ...
        for ( const condition of conditions_with_their_project_search_ids_for_First_condition_group ) {
            ...
            experiment_SubData_ForCondition.nsaf = nsaf;
        }
    }
}