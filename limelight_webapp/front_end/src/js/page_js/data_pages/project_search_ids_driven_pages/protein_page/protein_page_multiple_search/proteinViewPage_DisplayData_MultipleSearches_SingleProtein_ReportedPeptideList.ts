...
import { ProteinViewPage_Display_MultipleSearches_SingleProtein } from 'page_js/data_pages/project_search_ids_driven_pages/protein_page/protein_page_multiple_search/proteinViewPage_DisplayData_MultipleSearches_SingleProtein';
...
export class ProteinViewPage_DisplayData_MultipleSearches_SingleProtein_ReportedPeptideList {
	...
	private _containing_ProteinViewPage_Display_MultipleSearches_SingleProtein : ProteinViewPage_Display_MultipleSearches_SingleProtein;
	...
	private _current_reportedPeptideDisplayData : { peptideList, peptideItems_Map_Key_peptideSequenceDisplayString, numberOfReportedPeptides, numberOfPsmsForReportedPeptides };
	...
	constructor( 
			{ containing_ProteinViewPage_Display_MultipleSearches_SingleProtein,
				...
				containing_ProteinViewPage_Display_MultipleSearches_SingleProtein : ProteinViewPage_Display_MultipleSearches_SingleProtein,
	...
	_createReportedPeptideDisplayData( { 
		...
		peptideList, 
		peptideItems_Map_Key_peptideSequenceDisplayString, 
		...
		for ( const projectSearchId of projectSearchIds ) {
			...
			const variableModificationsRoundedArray_KeyPosition_KeyReportedPeptideId : Map<number, Map<number, Array<string>>> = (
				this._getVariableModificationsRoundedByReportedPeptideIdPosition_ForSingleProjectSearchId({ loadedDataPerProjectSearchIdHolder, reportedPeptideIdsForDisplay, proteinSequenceVersionId, projectSearchId })
			);
			...
			if ( reporterIonMassesSelected ) {
				...
				if ( ! psmReporterIonMassesPerPSM_ForPsmIdMap_ForReportedPeptideIdMap_CurrentCutoffs ) {
					let reportedPeptideIdsForDisplay_Display = "reportedPeptideIdsForDisplay empty";
					if ( reportedPeptideIdsForDisplay ) {
						reportedPeptideIdsForDisplay_Display = ""
						try {
							for ( const entry of reportedPeptideIdsForDisplay ) {
								reportedPeptideIdsForDisplay_Display += ", " + entry;
							}
						} catch (e) {
							reportedPeptideIdsForDisplay_Display = "Get reportedPeptideIdsForDisplay as string failed";
						}
					}
					throw Error("Multiple Searches: _createReportedPeptideDisplayData: reporterIonMassesSelected is populated. nothing returned from loadedDataPerProjectSearchIdHolder.get_psmReporterIonMassesPerPSM_ForPsmIdMap_ForReportedPeptideIdMap_CurrentCutoffs(): projectSearchId: " 
					+ projectSearchId 
					+ ", proteinSequenceVersionId: " + proteinSequenceVersionId
					+ ", reportedPeptideIdsForDisplay_Display: " + reportedPeptideIdsForDisplay_Display );
				}
			}
			...
			for ( const reportedPeptideId of reportedPeptideIdsForDisplay ) {
				...
				const peptideSequenceString : string = this._loadedDataCommonHolder.get_peptideSequenceString_For_peptideId( { peptideId } );
	...
	_getVariableModificationsRoundedByReportedPeptideIdPosition_ForSingleProjectSearchId({ loadedDataPerProjectSearchIdHolder, reportedPeptideIdsForDisplay, proteinSequenceVersionId, projectSearchId } : { 
	...
	}) : Map<number, Map<number, Array<string>>> {
		...
		for ( const proteinCoverageEntries_PerReportedPeptideId_Entry of proteinCoverageEntries_PerReportedPeptideId_Array ) {
			...
			if ( dynamicModificationsOnReportedPeptideArray && dynamicModificationsOnReportedPeptideArray.length > 0 ) {
				...
				for ( const dynamicModificationOnReportedPeptide of dynamicModificationsOnReportedPeptideArray ) {
					...
					//  N and C terminal mods are placed at the first and last positions of the peptides.

					//    Since some search programs report those mods at the first and last positions, 
					//    always place the mods at first and last positions for more matches of Generated Reported Peptide across Search Programs.

					//   Currently Not Using is_C_Terminal and is_N_Terminal (See comment immediately above)
		...
		const modsRoundedArray_KeyPosition_KeyReportedPeptideId : Map<number, Map<number, Array<string>>> = new Map(); // Map<(reported peptide), Map<(position),Set<(mod mass rounded sorted, to strings )>>
		...
		for ( const modsRoundedSet_KeyPosition_KeyReportedPeptideId_Entry of modsRoundedSet_KeyPosition_KeyReportedPeptideId.entries() ) {
			...
			const modsRoundedArray_KeyPosition_KeyReportedPeptideId_Entry : Map<number, Array<string>> = new Map();
	...
	// 	if ( ! dynamicModificationsOnReportedPeptide_KeyReportedPeptideId && dynamicModificationsOnReportedPeptide_KeyReportedPeptideId < 1 ) {
