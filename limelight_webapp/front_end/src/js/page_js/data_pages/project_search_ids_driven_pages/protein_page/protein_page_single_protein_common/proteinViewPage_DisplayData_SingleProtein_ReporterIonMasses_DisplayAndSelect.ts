/**
 * proteinViewPage_DisplayData_SingleProtein_ReporterIonMasses_DisplayAndSelect.ts
 * 
...
import { Handlebars, _protein_table_template_bundle } from './proteinViewPage_DisplayData_SingleProtein_ReporterIonMasses_DisplayAndSelect_ImportHandlebarsTemplates';
...
export class ProteinViewPage_DisplayData_SingleProtein_ReporterIonMasses_DisplayAndSelect {
    ...
    private _reporterIonMassTransformer;
    ...
    private _protein_reporter_ions_selection_block_Template = _protein_table_template_bundle.protein_reporter_ions_selection_block;
    private _protein_reporter_ions_selection_entry_Template = _protein_table_template_bundle.protein_reporter_ions_selection_entry;

    private _reporterIonMassesSelected; // = new Set();  // call .clear() to reset the selected

    private _reporterIonMasses_For_AllSearches_Set;
	...
	constructor({ 
        ...
        this._reporterIonMassesSelected = new Set();  // call .clear() to reset the selected
	...
	initialize({ encodedStateData, initial_reporterIonMassesSelected }) {
		...
		if ( initial_reporterIonMassesSelected ) {
            ...
            this._reporterIonMassesSelected = initial_reporterIonMassesSelected;
            
		} else if ( ! encodedStateData ) {
			this._reporterIonMassesSelected.clear(); // Reset to None
        }
	...
	getEncodedStateData() {
		...
		if (this._reporterIonMassesSelected && this._reporterIonMassesSelected.size !== 0) {

            const reporterIonMassesSelected_Array = Array.from( this._reporterIonMassesSelected );
	...
	_updateWithEncodedStateData({ encodedStateData }) {
        ...
        if ( encodedStateData[ _ENCODED_DATA__REPORTER_ION_MASSES_SELECTED_OBJECT_AND_ARRAY_ENCODING_PROPERTY_NAME ] ) {
            ...
            this._reporterIonMassesSelected = new Set( encodedStateData[ _ENCODED_DATA__REPORTER_ION_MASSES_SELECTED_OBJECT_AND_ARRAY_ENCODING_PROPERTY_NAME ] );
        } else {
            this._reporterIonMassesSelected = new Set();
        }
	}
    ...
    isAnyReporterIonMassSelected() {
        return this._reporterIonMassesSelected.size !== 0;
    }
    ...
    isReporterIonMassSelected( mass ) {
        return this._reporterIonMassesSelected.has( mass );
    }
    ...
    getReporterIonMassesSelected() {
        const selectionCopy = new Set( this._reporterIonMassesSelected );
	...
	_reporterIon_Display_Internal() {
        ...
        for ( const reporterIonMass of reporterIonMasses_ForSearch_Array ) {
            ...
            const entryContext = { reporterIonMass, isSelected : undefined };

            entryContext.isSelected = this._reporterIonMassesSelected.has( reporterIonMass );
    ...
    _reporterIonMassOnPageCheckboxChanged({ clickThis, eventObject, reporterIonMass }) {
        ...
        if ( $clickThis.prop('checked') ) {
            ...
            this._reporterIonMassesSelected.add( reporterIonMass );

        } else {
            this._reporterIonMassesSelected.delete( reporterIonMass );
        }
