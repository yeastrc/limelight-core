...
const LORIKEET_VIEWER_SIZE_PARAM_FOR_NEW_WINDOW_WIDTH_PARAM = 500;
const LORIKEET_VIEWER_SIZE_PARAM_FOR_NEW_WINDOW_HEIGHT_PARAM = 500;
...
//		console.log( "_psm_list_template_bundle: " + _psm_list_template_bundle );
	...
	_loadSpectrumData( { psmId, projectSearchId, loadedDataFromServer, newWindow } ) {
		...
		return new Promise( function( resolve, reject ) {
			...
			let _URL = "d/rws/for-page/psb/spectrum-for-psm-id/" + getWebserviceSyncTrackingCode();
			...
			$.ajax({
				...
				error : function(jqXHR, textStatus, errorThrown) {
					try {
						if ( newWindow ) {
							newWindow.close(); // close here before call handleAJAXError(...) since that may reload the page
						}
					} catch( e ) {
						reportWebErrorToServer.reportErrorObjectToServer( { errorException : e } );
						throw e;
	...
	_loadPSMPeptideData( { psmId, projectSearchId, loadedDataFromServer, searchDetailsBlockDataMgmtProcessing, dataPageStateManager_DataFrom_Server, newWindow } ) {
		...
		return new Promise( function( resolve, reject ) {
			...
			let _URL = "d/rws/for-page/psb/psm-peptide-list-display-with-spectrum-viewer/" + getWebserviceSyncTrackingCode();
