/**
 * projectPage_UploadData_ListExistingUploads.ts
 * 
...
import { ProjectPage_UploadData_UpdateExistingUploads } from './projectPage_UploadData_UpdateExistingUploads';
...
export class ProjectPage_UploadData_ListExistingUploads {
	...
	private statusIdQueued = undefined;
	private statusIdRequeued = undefined;
	private statusIdStarted = undefined;
	private statusIdComplete = undefined;
	private statusIdFailed = undefined;

	private prevCompleteSuccessTrackingIdList = undefined;
	private autoRefreshTimerId = undefined;

	private _file_import_item_row_details_template = _project_page_upload_data_section_project_owner_user_interaction_template.file_import_item_row_details_template;
	private _file_import_item_separator_template = _project_page_upload_data_section_project_owner_user_interaction_template.file_import_item_separator_template;
	private _file_import_item_template = _project_page_upload_data_section_project_owner_user_interaction_template.file_import_item_template;

	private _projectPage_UploadData_UpdateExistingUploads : ProjectPage_UploadData_UpdateExistingUploads;
...
/**
	...
	initialize() {
		...
		try {
			...
			this.populatePendingCount( undefined );
	...
	showUploadDataClicked(clickThis, eventObject) {
		...
		this.populateDataBlockAndPendingCount( undefined );
	}
	...
	populateDataBlockAndPendingCount(params) {
		...
		promise_webserviceCallStandardPost.then( ({ responseData }) => {
			try {
				objectThis.populateDataBlockAndPendingCountProcessResponse({ responseData });
			} catch (e) {
				reportWebErrorToServer.reportErrorObjectToServer({
					errorException : e
				});
	...
	populateDataBlockAndPendingCountProcessResponse( { responseData } ) {
		...
		//  Typescript throws error but currently working so ignore for now
		// @ts-ignore
	...
	populateDataBlockPendingOrHistoryItem( { dataItem, scanFileImportAllowedViaWebSubmit, $containerTable }) {
		...
		addToolTips( $upload_data_details_item );  // External Function
