    ...
    private _process_WebserviceResponse({responseData}: { responseData: any }): void {
        ...
        for (const entry of data_Array) {
            ...
            if (!limelight__variable_is_type_number_Check(entry.searchScanFileId)) {
                const msg = "( ! limelight__variable_is_type_number_Check( entry.searchScanFileId ) )";
                console.warn(msg);
                throw Error(msg);
            }
            ...
            if (!limelight__variable_is_type_number_Check(entry.projectScanFileId)) {
                const msg = "( ! limelight__variable_is_type_number_Check( entry.projectScanFileId ) )";
                console.warn(msg);
                throw Error(msg);
            }
        }
