/**
 * psm_DrilldownRetrieveDisplay.ts
 * 
...
import { Handlebars, _psm_list_template_bundle } from './psm_DrilldownRetrieveDisplay_ImportHandlebarsTemplates';
...
export class Psm_DrilldownRetrieveDisplay {
	...
	private _psm_list_table_headers_template_Template = _psm_list_template_bundle.psm_list_table_headers_template;

	private _psm_list_table_entry_template_Template = _psm_list_template_bundle.psm_list_table_entry_template;

	private _spectrumRetrieveAndDisplay_Use_lorikeet = new SpectrumRetrieveAndDisplay_Use_lorikeet();
...
//		console.log( "_psm_list_template_bundle: " + _psm_list_template_bundle );
	...
	loadAndDisplayPSMsIfNeeded(  { container, projectSearchId, reportedPeptideId }  ) {
		...
		if ( ! loadGoogleChart_CoreChartResult.isLoaded ) {
            loadGoogleChart_CoreChartResult.loadingPromise.catch( ( reason ) => {  })
		
			loadGoogleChart_CoreChartResult.loadingPromise.then( ( result ) => { 
				try {
	...
	_loadPSMsFromServer( { container, projectSearchId, reportedPeptideId } ) {
		...
		let psmAnnotationTypeIdsForSorting : Array<number> =
		...
		promise_webserviceCallStandardPost.then( ({ responseData }) => {
			try {
				...
				this._loadPSMsFromServerResponse( { projectSearchId, requestObject, responseData, container } );
	...
	_renderToPage( { projectSearchId, requestObject, responseData, container } ) {
		...
		if (psmList && psmList.length > 0) {
			...
			let annotationTypeRecords_DisplayOrder : { psmAnnotationTypesForPsmListEntries : Array<AnnotationTypeItem> } = this._getAnnotationTypeRecords_DisplayOrder( { projectSearchId, psmList } );
...
//			addToolTips();
	 ...
	 * 
	 */
	private _getAnnotationTypeRecords_DisplayOrder( { psmList, projectSearchId } ) {
		...
		const annotationTypeDataForProjectSearchId : AnnotationTypeItems_PerProjectSearchId = annotationTypeData_Root.annotationTypeItems_PerProjectSearchId_Map.get( projectSearchId );
		if ( ( ! annotationTypeDataForProjectSearchId ) ) {
			throw Error("No annotation type data for projectSearchId: " + projectSearchId );
		}
		
		let allPSMAnnotationTypeIds_InPsmList_Set : Set<number> = new Set();
			...
			if ( psmAnnotationMap ) {
				Object.keys ( psmAnnotationMap ).forEach( function( psmAnnotationMapKeyItem, index, array ) {
					...
					if ( ! variable_is_type_number_Check( psmAnnotationDTOItem.annotationTypeId ) ) {
						const msg = " psmAnnotationDTOItem.annotationTypeId is not a number: " +  psmAnnotationDTOItem.annotationTypeId;
						console.warn( msg );
						throw Error( msg );
					}
		...
		const psmAnnotationTypesForPsmListEntries : Array<AnnotationTypeItem> = [];
		...
		if ( allPSMAnnotationTypeIds_InPsmList_Set.size > 0 ) {
			...
			let psmDescriptiveAnnotationTypes_Map : Map<number, AnnotationTypeItem> = annotationTypeDataForProjectSearchId.psmDescriptiveAnnotationTypes
			...
			for ( const allPSMAnnotationTypeIds_InPsmList_Set_Entry of allPSMAnnotationTypeIds_InPsmList_Set ) {
				let annotationTypeEntryForKey = psmFilterableAnnotationTypes_Map.get( allPSMAnnotationTypeIds_InPsmList_Set_Entry );
				if ( ! annotationTypeEntryForKey ) {
					annotationTypeEntryForKey = psmDescriptiveAnnotationTypes_Map.get( allPSMAnnotationTypeIds_InPsmList_Set_Entry );
					if ( ! annotationTypeEntryForKey ) {
						throw Error( "No psmFilterableAnnotationTypes or psmDescriptiveAnnotationTypes entry for AnnotationTypeId: " + allPSMAnnotationTypeIds_InPsmList_Set_Entry );
					}
				}
		...
		this._sort_AnnotationTypes_OnDisplayOrderAnnTypeName( psmAnnotationTypesForPsmListEntries );
	 ...
	 * 
	 */
	_sort_AnnotationTypes_OnDisplayOrderAnnTypeName( annTypesArray : Array<AnnotationTypeItem> ) {
	 ...
	 * 
	 */
	_get_Psm_AnnotationTypeIds_WhereSortOrderPopulated( { projectSearchId } ) : Array<number> {

		let psmFilterableAnnotationTypes_WhereSortOrderPopulated : Array<AnnotationTypeItem> = 
	 ...
	 * Return array ann type entries, sorted on sortOrder
	 */
	_get_Psm_AnnotationTypeRecords_WhereSortOrderPopulated( { projectSearchId } ) : Array<AnnotationTypeItem> {
		...
		{
			for ( const psmFilterableAnnotationTypes_Map_Entry of psmFilterableAnnotationTypes_Map.entries() ) { 
				let annotationTypeEntryForKey = psmFilterableAnnotationTypes_Map_Entry[ 1 ]; // Value of Map Entry
		...
		{
			for ( const psmDescriptiveAnnotationTypes_Map_Entry of psmDescriptiveAnnotationTypes_Map.entries() ) { 
				let annotationTypeEntryForKey = psmDescriptiveAnnotationTypes_Map_Entry[ 1 ]; // Value of Map Entry
				if ( annotationTypeEntryForKey.sortOrder ) {
					psmAnnotationTypes_SortOrderPopulated.push( annotationTypeEntryForKey );
				}
			}
	...
	_addPsmChartsActual( { psmList, container }  ) {
		...
		//   window.linkInfoOverlayWidthResizer does not exist
...
//			, { role: 'style' } 
		...
		for ( let index = 0; index < buckets.length; index++ ) {
			...
			let chartXaxisValue : string = approxBucketCenter.toString();
