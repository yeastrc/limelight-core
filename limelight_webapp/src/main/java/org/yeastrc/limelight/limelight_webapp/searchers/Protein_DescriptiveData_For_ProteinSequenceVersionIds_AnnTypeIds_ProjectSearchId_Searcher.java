...
import org.yeastrc.limelight.limelight_webapp.dao.Search_Protein_DescriptiveAnnotationDAO_IF;
 ...
 */
@Component
public class Protein_DescriptiveData_For_ProteinSequenceVersionIds_AnnTypeIds_ProjectSearchId_Searcher extends Limelight_JDBC_Base implements Protein_DescriptiveData_For_ProteinSequenceVersionIds_AnnTypeIds_ProjectSearchId_Searcher_IF   {

	private static final Logger log = LoggerFactory.getLogger( Protein_DescriptiveData_For_ProteinSequenceVersionIds_AnnTypeIds_ProjectSearchId_Searcher.class );
	
	@Autowired
	private Search_Protein_DescriptiveAnnotationDAO_IF search_Protein_DescriptiveAnnotationDAO;
	
	private final String SQL_MAIN = 
			"SELECT *  "

			+ " FROM srch__protein_descriptive_annotation_tbl "
	 ...
	 */
	@Override
	public List<Search_Protein_DescriptiveAnnotation_DTO>  get_Protein_DescriptiveAnnDataList( 
			int searchId, 
    		List<Integer> proteinSequenceVersionIds,
    		List<Integer> annotationTypeIds ) throws SQLException {

		List<Search_Protein_DescriptiveAnnotation_DTO> resultList = new ArrayList<>( 10000 );
			...
			try ( ResultSet rs = pstmt.executeQuery() ) {
				while( rs.next() ) {
					Search_Protein_DescriptiveAnnotation_DTO item = 
							search_Protein_DescriptiveAnnotationDAO.populateFromResultSet( rs );
					resultList.add( item );
				}
			}
