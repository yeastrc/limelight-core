 ...
 */
@RestController
public class Scan_Count_ForSearchCriteria_Single_ProjSearchID_RestWebserviceController 

implements
InitializingBean // InitializingBean is Spring Interface for triggering running method afterPropertiesSet() 
{
  
	private static final Logger log = LoggerFactory.getLogger( Scan_Count_ForSearchCriteria_Single_ProjSearchID_RestWebserviceController.class );
	 ...
	 */
	private static final String CONTROLLER_PATH = AA_RestWSControllerPaths_Constants.SCAN_COUNT_SEARCHCRITERIA_REST_WEBSERVICE_CONTROLLER_VERSION_0002;
	 ...
	 * 
	 */
	public Scan_Count_ForSearchCriteria_Single_ProjSearchID_RestWebserviceController() {
		super();
...
//    		ValidateWebSessionAccess_ToWebservice_ForAccessLevelAndProjectSearchIds_Result validateWebSessionAccess_ToWebservice_ForAccessLevelAndProjectSearchIds_Result =
    		...
    		/**
        	 * Map< search_scan_file_id, Set< Scan_Number > >
        	 */
    		Map<Integer, Set<Integer>> scanNumbers_Map_Key_SearchScanFileId = new HashMap<>();

    		for ( ReportedPeptide_MinimalData_List_FromSearcher_Entry reportedPeptide_MinimalData_List_FromSearcher_Entry : peptideMinimalObjectsList ) {
    			...
    			for ( PsmWebDisplayWebServiceResult psmWebDisplayWebServiceResult : psmWebDisplayWebServiceResult_List ) {
    				...
    				Set<Integer> scanNumbers = scanNumbers_Map_Key_SearchScanFileId.get( searchScanFileId );
    				if ( scanNumbers == null ) {
    					scanNumbers = new HashSet<>();
    					scanNumbers_Map_Key_SearchScanFileId.put( searchScanFileId, scanNumbers );
    				}
    				
    				scanNumbers.add( psmWebDisplayWebServiceResult.getScanNumber() );
    			}
    		}
    		...
    		for ( Map.Entry<Integer, Set<Integer>> entry : scanNumbers_Map_Key_SearchScanFileId.entrySet() ) {
    			
    			Set<Integer> scanNumbers = entry.getValue();
    			scanCount += 	scanNumbers.size();	
    		}
