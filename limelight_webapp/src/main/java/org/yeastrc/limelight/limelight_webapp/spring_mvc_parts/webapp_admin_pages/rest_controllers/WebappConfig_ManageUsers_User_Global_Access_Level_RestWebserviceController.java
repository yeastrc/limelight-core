...
/**
 * User Management - User - Global Access Level
 *
 */
@RestController
public class WebappConfig_ManageUsers_User_Global_Access_Level_RestWebserviceController {

	private static final Logger log = LoggerFactory.getLogger( WebappConfig_ManageUsers_User_Global_Access_Level_RestWebserviceController.class );
	...
	@PostMapping( 
			path = { 
					...
					+ AA_WebappAdmin_RestWSControllerPaths_Constants.WEBAPP_ADMIN_MANAGE_USERS_USER_GLOBAL_ACCESS_LEVEL_REST_WEBSERVICE_CONTROLLER
			},
	...
	private WebserviceResult webserviceMethodInternal( WebserviceRequest webserviceRequest, String remoteIP, HttpServletRequest httpServletRequest ) throws Exception {
		...
		if ( StringUtils.isEmpty( personAccessLevelString ) ) {
			String msg = "No value for personAccessLevel";
			log.warn( msg );
			throw new Limelight_WS_BadRequest_InvalidParameter_Exception();
		}
		int personId = 0;
		...
		try {
			...
			String msg = "personAccessLevel not an integer, is: " + personAccessLevelString;
			log.warn( msg );
			throw new Limelight_WS_BadRequest_InvalidParameter_Exception();
		}
		
		//  Save values to DB
		userDAO.updateUserAccessLevel( personId, personAccessLevel );
