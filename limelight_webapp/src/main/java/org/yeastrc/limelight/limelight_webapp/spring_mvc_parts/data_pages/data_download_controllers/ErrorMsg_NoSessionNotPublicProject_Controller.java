...
/**
 * Forward to JSP with error message that there is an error due to no session.
 * 
 * 
 *
 */
@Controller
public class ErrorMsg_NoSessionNotPublicProject_Controller {

	private static final Logger log = LoggerFactory.getLogger( ErrorMsg_NoSessionNotPublicProject_Controller.class );
	

	private static final String PRIMARY_CONTROLLER_PATH = AA_DataDownloadControllersPaths_Constants.NO_SESSION_NOT_PUBLIC_PROJECT_DOWNLOAD_CONTROLLER;
	 ...
	 * @return
	 */
	@GetMapping( path = {CONTROLLER_PATH_MAIN} )

	public String controllerEntryMain_GET(
	...
	@PostMapping( path = {CONTROLLER_PATH_MAIN}  )

	public String controllerEntryMain_POST(
			HttpServletRequest httpServletRequest,
			HttpServletResponse httpServletResponse ) {
	...
	private String controllerEntryInternal( 
		...
		//  Should only get to this controller if no session but run this code anyway
        ...
        return "downloadError_NoSessionOrProjectNotPublic.jsp";  // forward to JSP. Path to JSP specified in application.properties:spring.mvc.view.prefix
