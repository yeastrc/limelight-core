...
public class FileUploadMaxFileSize_Config_WithConstantsDefaults {
	...
	public static long get_SCAN_MAX_FILE_UPLOAD_SIZE() {
		...
		int result_InGB = get_Max_Scan_FileSize_From_Environment_Or_JVM_dashD_Property__OrDefault();
	...
	private static int get_Max_LimelightXML_FileSize_From_Environment_Or_JVM_dashD_Property__OrDefault() {
		...
		{
			String value = System.getenv(MAX_LIMELIGHT_XML_FILE_UPLOAD_SIZE_DEFAULT_IN_GB__ENV_LABEL);
			if ( StringUtils.isNotEmpty(value) ) {
				try {
					...
					String errorMessage = "get_Max_LimelightXML_FileSize_From_Environment_Or_JVM_dashD_Property__OrDefault(): Value in environment variable '" 
							+ MAX_LIMELIGHT_XML_FILE_UPLOAD_SIZE_DEFAULT_IN_GB__ENV_LABEL
		...
		if ( result == null ) {
			...
			String value = prop.getProperty(MAX_LIMELIGHT_XML_FILE_UPLOAD_SIZE_DEFAULT_IN_GB__ENV_LABEL);
			if ( StringUtils.isNotEmpty(value) ) {
				try {
					result = Integer.parseInt(value);
				} catch (Throwable t) {
	...
	private static int get_Max_Scan_FileSize_From_Environment_Or_JVM_dashD_Property__OrDefault() {
		...
		{
			String value = System.getenv(MAX_SCAN_FILE_UPLOAD_SIZE_DEFAULT_IN_GB__ENV_LABEL);
			if ( StringUtils.isNotEmpty(value) ) {
				try {
					result = Integer.parseInt(value);
				} catch (Throwable t) {
					String errorMessage = "get_Max_Scan_FileSize_From_Environment_Or_JVM_dashD_Property__OrDefault(): Value in environment variable '" 
							+ MAX_SCAN_FILE_UPLOAD_SIZE_DEFAULT_IN_GB__ENV_LABEL
							+ "' is not parsable as integer.  value: " + value;
		...
		if ( result == null ) {
			...
			Properties prop = System.getProperties();
			String value = prop.getProperty(MAX_SCAN_FILE_UPLOAD_SIZE_DEFAULT_IN_GB__ENV_LABEL);
			if ( StringUtils.isNotEmpty(value) ) {
				try {
					result = Integer.parseInt(value);
				} catch (Throwable t) {
