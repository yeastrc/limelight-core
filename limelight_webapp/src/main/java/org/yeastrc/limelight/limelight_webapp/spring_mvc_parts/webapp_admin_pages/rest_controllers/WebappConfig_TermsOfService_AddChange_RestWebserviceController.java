...
/**
 * Add/Change Webapp Terms Of Service
 *
 */
@RestController
public class WebappConfig_TermsOfService_AddChange_RestWebserviceController {

	private static final Logger log = LoggerFactory.getLogger( WebappConfig_TermsOfService_AddChange_RestWebserviceController.class );
	...
	@PostMapping( 
			path = { 
					...
					+ AA_WebappAdmin_RestWSControllerPaths_Constants.WEBAPP_ADMIN_TERMS_OF_SERVICE_ADD_CHANGE_REST_WEBSERVICE_CONTROLLER
			},
	...
	private WebserviceResult webserviceMethodInternal( WebserviceRequest webserviceRequest, String remoteIP, HttpServletRequest httpServletRequest ) throws Exception {
		...
		if ( userId == null ) {
			String msg = "userSession.getUserId() is null";
			log.error( msg );
			throw new LimelightInternalErrorException(msg);
		}

		if ( StringUtils.isEmpty( webserviceRequest.termsOfServiceText ) ) {
			String msg = "No value for termsOfServiceText";
			log.warn( msg );
		...
		termsOfServiceTextVersionsDTO.setTermsOfServiceText( webserviceRequest.termsOfServiceText );
		termsOfServiceTextVersionsDTO.setCreatedUserId( userId );
		termsOfServiceTextVersionsDAO.save( termsOfServiceTextVersionsDTO );
