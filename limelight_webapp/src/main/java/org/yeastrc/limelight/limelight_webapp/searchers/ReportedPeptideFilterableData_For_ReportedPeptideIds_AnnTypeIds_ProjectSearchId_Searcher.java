 ...
 */
@Component
public class ReportedPeptideFilterableData_For_ReportedPeptideIds_AnnTypeIds_ProjectSearchId_Searcher extends Limelight_JDBC_Base implements ReportedPeptideFilterableData_For_ReportedPeptideIds_AnnTypeIds_ProjectSearchId_SearcherIF {

	private static final Logger log = LoggerFactory.getLogger( ReportedPeptideFilterableData_For_ReportedPeptideIds_AnnTypeIds_ProjectSearchId_Searcher.class );
	
	private final String SQL_MAIN = 
			"SELECT reported_peptide_id, annotation_type_id, value_double, value_string  "
	 ...
	 */
	@Override
	public List<SearchReportedPeptideFilterableAnnotationDTO>  getReportedPeptideFilterableAnnDataList( 
			int searchId, 
    		List<Integer> reportedPeptideIds,
    		List<Integer> annotationTypeIds ) throws SQLException {

		List<SearchReportedPeptideFilterableAnnotationDTO> resultList = new ArrayList<>( 10000 );
...
//			}
			...
			try ( ResultSet rs = pstmt.executeQuery() ) {
				while( rs.next() ) {
					SearchReportedPeptideFilterableAnnotationDTO item = new SearchReportedPeptideFilterableAnnotationDTO();
