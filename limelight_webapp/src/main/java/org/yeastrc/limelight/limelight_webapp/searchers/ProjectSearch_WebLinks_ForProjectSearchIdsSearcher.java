...
/**
 * WebLinks for Search Details (When Expand a Search)
 *
 */
@Component
public class ProjectSearch_WebLinks_ForProjectSearchIdsSearcher extends Limelight_JDBC_Base implements ProjectSearch_WebLinks_ForProjectSearchIdsSearcherIF  {

	private static final Logger log = LoggerFactory.getLogger( ProjectSearch_WebLinks_ForProjectSearchIdsSearcher.class );
	
	private static final String MAIN_QUERY_SQL = 
			" SELECT id, project_search_id, user_id, link_url, link_label "
			+ " FROM "
			+ " project_search__web_links_tbl "
	 ...
	 */
	@Override
	public List<ProjectSearchWebLinksDTO>  getProjectSearchWebLinksDTO_ForProjectSearchIds( List<Integer> projectSearchIds ) throws SQLException {

		if ( projectSearchIds == null || projectSearchIds.isEmpty() ) {
			throw new IllegalArgumentException( "projectSearchIds is null or empty" );
		}
		
		List<ProjectSearchWebLinksDTO> results = new ArrayList<>();
		...
		try ( Connection connection = super.getDBConnection();
			...
			try ( ResultSet rs = preparedStatement.executeQuery() ) {
				while ( rs.next() ) {
					ProjectSearchWebLinksDTO result = new ProjectSearchWebLinksDTO();
					...
					result.setLinkLabel( rs.getString( "link_label" ) );
					results.add(result);
				}
			}
