 ...
 */
@Component
public class FileImportAndPipelineRunTracking_All_Searcher extends Limelight_JDBC_Base implements FileImportAndPipelineRunTracking_All_Searcher_IF {

	private static final Logger log = LoggerFactory.getLogger( FileImportAndPipelineRunTracking_All_Searcher.class );
	...
	@Override
	public List<FileImportAndPipelineRunTrackingDTO> getAllForWebDisplayForProject( int projectId ) throws Exception {

		List<FileImportAndPipelineRunTrackingDTO> resultList = new ArrayList<>();

		final String querySQL =  "SELECT * FROM import_and_pipeline_run_tracking_tbl"
				+ " WHERE "
				+ SELECT_WHERE_CLAUSE;
		...
		try ( Connection connection = super.getDBConnection();
			...
			try ( ResultSet rs = preparedStatement.executeQuery() ) {
				...
				while( rs.next() ) {
					FileImportAndPipelineRunTrackingDTO returnItem = 
							FileImportAndPipelineRunTracking_PopulateDTO.getInstance()
							.populateResultObject( rs );
					resultList.add(returnItem);
				}
			}
		} catch ( Exception e ) {
			String msg = "Failed to select FileImportAndPipelineRunTrackingDTO, sql: " + querySQL;
			log.error( msg, e );
			throw e;
		}
		return resultList;
	...
	public List<FileTrackingIdStatusId> getAllStatusExceptInitInsertForProject( int projectId ) throws Exception {
		...
		final String querySQL =  "SELECT id, status_id FROM import_and_pipeline_run_tracking_tbl"
				+ " WHERE "
				+ SELECT_WHERE_CLAUSE;
