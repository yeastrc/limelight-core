...
import org.yeastrc.limelight.limelight_webapp.spring_mvc_parts.data_pages.rest_request_objects.controller_request_root.PeptideView_PeptideList_Single_ProjectSearchId_RequestRoot;
 ...
 */
@RestController
public class PeptideView_PeptideList_Single_ProjectSearchId_RestWebserviceController {
  
	private static final Logger log = LoggerFactory.getLogger( PeptideView_PeptideList_Single_ProjectSearchId_RestWebserviceController.class );
	 ...
	 * 
	 */
	public PeptideView_PeptideList_Single_ProjectSearchId_RestWebserviceController() {
		super();
//		log.warn( "constructor no params called" );
	...
	@PostMapping( 
			path = {
					...
					+ AA_RestWSControllerPaths_Constants.PEPTIDE_VIEW_PAGE_PEPTIDE_LIST_SINGLE_PROJECT_SEARCH_ID_REST_WEBSERVICE_CONTROLLER
			},
...
//    		log.warn( "peptideView(...) called" );
    		...
    		PeptideView_PeptideList_Single_ProjectSearchId_RequestRoot webserviceRequest = unmarshal_RestRequest_JSON_ToObject.getObjectFromJSONByteArray( postBody, PeptideView_PeptideList_Single_ProjectSearchId_RequestRoot.class );
...
//    		ValidateWebSessionAccess_ToWebservice_ForAccessLevelAndProjectSearchIds_Result validateWebSessionAccess_ToWebservice_ForAccessLevelAndProjectSearchIds_Result =
			...
			//  Get initial Peptide list based on search criteria
    		List<ReportedPeptideBasicObjectsSearcherResultEntry> peptideBasicObjectsList = peptideListForProjectSearchIdSearcher.getPeptideList( searchId, searcherCutoffValuesSearchLevel );

    		//  Final output Peptide List
    		List<PeptideItemForPeptideList_SingleProjectSearchId> peptideResultListResult = new ArrayList<>( peptideBasicObjectsList.size() );

    		for ( ReportedPeptideBasicObjectsSearcherResultEntry entry : peptideBasicObjectsList ) {
    			...
    			int reportedPeptideId = entry.getReportedPeptideId();
    			
    			ReportedPeptideDTO reportedPeptideDTO = reportedPeptideDAO.getForId( reportedPeptideId );
    			if ( reportedPeptideDTO == null ) {
    				String msg = "Should not get this here: reportedPeptideDTO == null for entry.getReportedPeptideId():  " + entry.getReportedPeptideId();
    				log.error( msg );
    			...
    			List<PeptideItem_ModMassEntry> modMassList = new ArrayList<>( srchRepPeptDynamicModDTOList.size() );
    			...
    			List<PeptideItem_ProteinNameEntry> proteinNameList = new ArrayList<>( proteinSequenceVersionIds.size() );
    			...
    			for ( Integer proteinSequenceVersionId : proteinSequenceVersionIds ) {
    				...
    				if ( StringUtils.isNotEmpty( proteinName ) ) {
    					...
    					String msg = "No protein name for searchId: " + searchId + ", proteinSequenceVersionId: " + proteinSequenceVersionId;
    					log.error( msg );
    					throw new LimelightInternalErrorException(msg);
    				}
    			}
    			...
    			Map<Integer, AnnotationDataItem_ForPage> peptideAnnotationMap =
    					translateSearcherAnnotationDataBaseDTO_To_WebserviceResultMap( entry.getPeptideAnnotationDTOMap() );
    				...
    				//  Add in Reported Peptide Annotation data for Ann Type Display
    				{
    					...
    					List<SearchReportedPeptideFilterableAnnotationDTO> searchReportedPeptideFilterableAnnotationDTOList =
    							...
    							.getSearchReportedPeptideFilterableAnnotationDTOList( 
    									searchId, entry.getReportedPeptideId(), annTypeIdsToRetrieve );
    			...
    			Map<Integer, AnnotationDataItem_ForPage> psmAnnotationMap =
    					translateSearcherAnnotationDataBaseDTO_To_WebserviceResultMap( entry.getPsmAnnotationDTOMap() );
    			...
    			peptideItemForProjectSearch.setReportedPeptideId( entry.getReportedPeptideId() );
...
//    			peptideItemForProjectSearch.setNumUniquePsms( entry.getNumUniquePsms() );
