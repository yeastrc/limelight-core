 ...
 */
@Component
public class FileObjectStorage_MainEntry_DAO_Webapp extends Limelight_JDBC_Base implements FileObjectStorage_MainEntry_DAO_Webapp_IF {

	private static final Logger log = LoggerFactory.getLogger( FileObjectStorage_MainEntry_DAO_Webapp.class );
	...
	public Integer get_Id_For_FileTypeId_AND_FileObjectStorageStorageAPIKey( int fileTypeId, String file_object_storage_api_key ) throws Exception {
		...
		try ( Connection dbConnection = super.getDBConnection();
			...
			preparedStatement.setInt( 1, fileTypeId );
			preparedStatement.setString( 2, file_object_storage_api_key );
			...
			try ( PreparedStatement pstmt = dbConnection.prepareStatement( sql, Statement.RETURN_GENERATED_KEYS ) ) {
				...
				try ( ResultSet rs = pstmt.getGeneratedKeys() ) {
					if( rs.next() ) {
						...
						throw new LimelightDatabaseException( "Failed to insert for " + item.getFileObjectStorageStorageAPIKey() );
				}
			}
		} catch ( Exception e ) {
