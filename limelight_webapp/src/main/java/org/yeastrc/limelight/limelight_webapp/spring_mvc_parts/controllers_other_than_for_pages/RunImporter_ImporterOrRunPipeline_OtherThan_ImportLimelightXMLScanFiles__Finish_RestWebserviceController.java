...
import org.yeastrc.limelight.limelight_webapp.services.SendEmailForRunImportFinish_ImportOrRunPipeline_Service_IF;
...
/**
 * Called by Run Importer (for submitted import or run pipeline) when import or run pipeline ( OTHER THAN: Limelight XML and/or Scan Files ) finishes
 *
 */
@RestController
public class RunImporter_ImporterOrRunPipeline_OtherThan_ImportLimelightXMLScanFiles__Finish_RestWebserviceController {

	private static final Logger log = LoggerFactory.getLogger( RunImporter_ImporterOrRunPipeline_OtherThan_ImportLimelightXMLScanFiles__Finish_RestWebserviceController.class );
	...
	@Autowired
	private SendEmailForRunImportFinish_ImportOrRunPipeline_Service_IF sendEmailForRunImportFinish_ImportOrRunPipeline_Service;
	
	
	@PostMapping( 
			path = { 
					AA_OtherRestControllerPaths_Constants.PATH_START_ALL
					+ AA_OtherRestControllerPaths_Constants.RUN_IMPORTER_IMPORT_OR_RUN_PIPELINE_FINISH_REST_WEBSERVICE_CONTROLLER
			},
...
//    				+ webserviceRequest.getTrackingId() + ", RunId: " + webserviceRequest.getRunId() );
    		...
    		FileImportAndPipelineRunTrackingDTO fileImportAndPipelineRunTrackingDTO =
    				FileImportAndPipelineRunTracking_Shared_Get_DAO.getInstance().getItem( webserviceRequest.getTrackingId() );
    		if ( fileImportAndPipelineRunTrackingDTO == null ) {
    			String msg = "fileImportAndPipelineRunTrackingDTO == null: webserviceRequest.trackingId: " 
    					+ webserviceRequest.getTrackingId() ;
    			log.warn( msg );
    			throw new Limelight_WS_BadRequest_InvalidParameter_Exception();
    		}
    		FileImportAndPipelineRunTrackingRunDTO fileImportAndPipelineRunTrackingRunDTO =
    				FileImportAndPipelineRunTrackingRun_Shared_Get_DAO.getInstance().getItem( webserviceRequest.getRunId() );
    		if ( fileImportAndPipelineRunTrackingRunDTO == null ) {
    			String msg = "fileImportAndPipelineRunTrackingRunDTO == null: webserviceRequest.runId: "
    					+ webserviceRequest.getRunId() ;
    			log.warn( msg );
    			throw new Limelight_WS_BadRequest_InvalidParameter_Exception();
    		}

    		sendEmailForRunImportFinish_ImportOrRunPipeline_Service
    		.sendEmailForRunImportFinish_ImportOrRunPipeline_Service( fileImportAndPipelineRunTrackingDTO, fileImportAndPipelineRunTrackingRunDTO );
