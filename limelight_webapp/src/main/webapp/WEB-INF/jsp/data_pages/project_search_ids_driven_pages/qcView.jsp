<%--
	...
	View qc page for a project search or multiple
...
<head>
	...
	<script id="controller_path" type="text/text"><%= AA_PageControllerPaths_Constants.QC_VIEW_PAGE_CONTROLLER %></script>
 ...
 <%@ include file="/WEB-INF/jsp/jsp_includes_body_start_body_end/body_after_body_tag_start_include_data_pages.jsp" %> 
  ...
  <%@ include file="/WEB-INF/jsp/jsp_header_footer/header_main_pages.jsp" %>
   ...
   <%@ include file="/WEB-INF/jsp/jsp_includes_body_start_body_end/body_after_header_include_data_pages.jsp" %>
	...
	<div 
		id="main_qc_view_loading_data_root_container" 
		style="font-size: 18px; margin-top: 20px; margin-bottom: 20px;"
	>
		LOADING DATA
	</div>
		
	<!--  Main QC View Container -->
	<div id="main_qc_view_outer_block_react_root_container"></div>
 ...
 <%@ include file="/WEB-INF/jsp/jsp_includes_body_start_body_end/body_before_closing_body_tag_include_data_pages.jsp" %>
  ...
  <c:choose>
    <c:when test="${ webSessionAuthAccessLevel.assistantProjectOwnerAllowed }">
	  ...
	  <script type="text/javascript" src="static/js_generated_bundles/data_pages/qcViewPage_RootLaunch_LoggedInUsers-bundle.js?x=${ cacheBustValue }"></script>
  	</c:when>
  	<c:otherwise>
	  	<%--  Public User or Project is Locked --%>
	  <script type="text/javascript" src="static/js_generated_bundles/data_pages/qcViewPage_RootLaunch_PublicUser-bundle.js?x=${ cacheBustValue }"></script>
  	</c:otherwise>
  </c:choose>
