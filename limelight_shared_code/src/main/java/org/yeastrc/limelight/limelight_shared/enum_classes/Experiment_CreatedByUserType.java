...
/**
 * Enum for is this Experiment record for Created By User Type 
 * 
 * Keep these values in sync with the enum field 'created_by_user_type' 
 * in the tables experiment+tbl  
 */
public enum Experiment_CreatedByUserType {
     ...
     */
    private Experiment_CreatedByUserType(String v) {
        value = v;
    }
     ...
     * @return
     */
    public static Experiment_CreatedByUserType fromValue( String value_ ) {
        for (Experiment_CreatedByUserType c: Experiment_CreatedByUserType.values()) {
            if (c.value.equals( value_ )) {
                return c;
            }
        }
        throw new IllegalArgumentException( "Experiment_CreatedByUserType not valid for value: " + value_ );
    }
