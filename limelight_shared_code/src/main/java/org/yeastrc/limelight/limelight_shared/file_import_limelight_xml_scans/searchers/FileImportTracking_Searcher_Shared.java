...
public class FileImportTracking_Searcher_Shared {
	...
	public int getItemCount_Status_QUEUED__RE_QUEUED() throws Exception {
		...
		ResultSet rs = null;
		final String sql = "SELECT COUNT(*) AS count FROM file_import_tracking_tbl WHERE status_id IN ( " + FileImportStatus.QUEUED.value() + ", " + FileImportStatus.RE_QUEUED.value() + ")";
		try {
