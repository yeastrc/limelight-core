 ...
 *
 */
public class Limelight_DatabaseCleanup_ProteinSequenceVersionTbl_DAO_Searcher {

	private static final Logger log = LoggerFactory.getLogger(Limelight_DatabaseCleanup_ProteinSequenceVersionTbl_DAO_Searcher.class);
	private Limelight_DatabaseCleanup_ProteinSequenceVersionTbl_DAO_Searcher() { }
	public static Limelight_DatabaseCleanup_ProteinSequenceVersionTbl_DAO_Searcher getInstance() { return new Limelight_DatabaseCleanup_ProteinSequenceVersionTbl_DAO_Searcher(); }
	...
	private static final String SELECT_COUNT_START = "SELECT COUNT( protein_sequence_version_tbl.id ) AS count FROM protein_sequence_version_tbl  INNER JOIN (  ";

	private static final String DELETE_START = "DELETE protein_sequence_version_tbl FROM protein_sequence_version_tbl INNER JOIN (  ";
	
	
	private static final String INNER_SUB_SELECT = 
			"SELECT protein_sequence_version_tbl.id FROM protein_sequence_version_tbl "
					+ " INNER JOIN protein_sequence_tbl ON protein_sequence_version_tbl.protein_sequence_id = protein_sequence_tbl.id "
					+ " LEFT OUTER JOIN srch__prot_seq_v_id_tbl ON protein_sequence_version_tbl.id = srch__prot_seq_v_id_tbl.protein_sequence_version_id "
					+ " WHERE "
					...
					+ " AND "
					+ " srch__prot_seq_v_id_tbl.protein_sequence_version_id IS NULL ";  // not found in srch__prot_seq_v_id_tbl

	private static final String INNER_SUB_SELECT__ID_RANGE = " AND protein_sequence_version_tbl.id > ? AND protein_sequence_version_tbl.id <= ? "; 
	
	private static final String LIMIT_CLAUSE = " LIMIT ? ";
	
	private static final String OUTER_DELETE_FINAL_PART = ") AS ids_to_delete ON ids_to_delete.id = protein_sequence_version_tbl.id";
	...
	public int getMaxId( ) throws Exception {
		...
		final String sql = "SELECT MAX(id) AS max_id FROM protein_sequence_version_tbl ";
	 ...
	 *
	 */
	public static class Limelight_DatabaseCleanup_ProteinSequenceVersionTbl_DAO_Searcher_deleteBlock_Params {
	 ...
	 *
	 */
	public static class Limelight_DatabaseCleanup_ProteinSequenceVersionTbl_DAO_Searcher_deleteBlock_Result {
	 ...
	 * @throws Exception
	 */
	public Limelight_DatabaseCleanup_ProteinSequenceVersionTbl_DAO_Searcher_deleteBlock_Result deleteBlock( Limelight_DatabaseCleanup_ProteinSequenceVersionTbl_DAO_Searcher_deleteBlock_Params params ) throws Exception {
		...
		if ( params.callFrom == Limelight_DatabaseCleanup__CallFrom__RunImporter_VS_StandaloneProgram_Enum.STANDALONE_PROGRAM ) {
			...
			System.out.println( "START: " + new Date()  
					+ ": Deleting record from table 'protein_sequence_version_tbl':  "
		...
		Limelight_DatabaseCleanup_ProteinSequenceVersionTbl_DAO_Searcher_deleteBlock_Result result = new Limelight_DatabaseCleanup_ProteinSequenceVersionTbl_DAO_Searcher_deleteBlock_Result();
