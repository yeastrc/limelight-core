 ...
 *
 */
public class FileImportAndPipelineRunTracking_For_ImporterRunner_DAO {

	private static final Logger log = LoggerFactory.getLogger( FileImportAndPipelineRunTracking_For_ImporterRunner_DAO.class );

	//  private constructor
	private FileImportAndPipelineRunTracking_For_ImporterRunner_DAO() { }
	
	/**
	 * @return newly created instance
	 */
	public static FileImportAndPipelineRunTracking_For_ImporterRunner_DAO getInstance() { 
		return new FileImportAndPipelineRunTracking_For_ImporterRunner_DAO(); 
	}
	
	
	private static final String GET_NEXT_QUEUED_SQL = 
			
			"SELECT * FROM import_and_pipeline_run_tracking_tbl "
			+ " WHERE status_id IN ( " 
	 ...
	 * @throws Exception
	 */
	public FileImportAndPipelineRunTrackingDTO getNextQueued( int maxPriority, Connection dbConnection ) throws Exception {
		...
		try {
			...
			if ( rs.next() ) {
				...
				result = FileImportAndPipelineRunTracking_PopulateDTO.getInstance().populateResultObject( rs );
			}
